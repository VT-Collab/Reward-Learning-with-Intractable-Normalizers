...
def mcmc_naive(a_h, n_outer_samples, n_burn):
    ...
    for _ in range(n_outer_samples):
        ...
        p_theta1 = reward(a_h, theta1) #reward comparison
...
def mcmc_mean(a_h, n_outer_samples, n_inner_samples, n_burn):
    #Generate random theta
    
    t = np.random.rand()*np.pi/2
    theta = np.array([np.cos(t), np.sin(t)])
    
    Z_theta = Z_mean(theta, n_inner_samples) #normalizer found by mean reward found in inner sampler
   
    #test theta reward for ideal human action
    p_theta = reward(a_h, theta) / Z_theta 
    theta_samples = []
    for _ in range(n_outer_samples):
        theta_samples.append(theta)
        ...
        a = time.time()
        Z_theta1 = Z_mean(theta1, n_inner_samples)
        b = time.time() - a

        p_theta1 = reward(a_h, theta1) / Z_theta1
...
def mcmc_max(a_h, n_outer_samples, n_inner_samples, n_burn):
    #Generate random theta
    
    t = np.random.rand()*np.pi/2
    theta = np.array([np.cos(t), np.sin(t)])
    
    Z_theta = Z_max(theta, n_inner_samples) #normalizer found by max reward found in inner sampler
    ...
    for _ in range(n_outer_samples):
        ...
        a = time.time() 
        Z_theta1 = Z_max(theta1, n_inner_samples)
        b = time.time() - a
...
def mcmc_double(a_h, n_outer_samples, n_inner_samples, n_burn):
    #Generate random theta
    ...
    for _ in range(n_outer_samples):
        ...
        x_theta = reward(a_h, theta)
        x_theta1 = reward(a_h, theta1)
        #reward for sampled inner action to compare based off ideal inner sample
        y_theta = reward(y, theta) 
        y_theta1 = reward(y, theta1)
...
def Z_mean(theta, n_samples):
    mean_reward = 0.
    for _ in range(n_samples):
        a = rand_action()
        mean_reward += reward(a, theta)
    return mean_reward / n_samples

#Max approach normalizer
def Z_max(theta, n_samples):
    max_reward = -np.inf
    for _ in range(n_samples):
        a = rand_action()
        r = reward(a, theta)
...
def inner_sampler(theta, n_samples):
    a = rand_action()
    a_score = reward(a, theta) #baseline action and score for theta 
    for _ in range(n_samples):
        a1 = rand_action() #redo random sample with new random action for random theta sample
        a1_score = reward(a1, theta)
