...
class Env1():
    ...
    def __init__(self, block_position=[0.55, 0.1, 0.2], goal_position=[.35,-.35,.2], visualize=True):
        ...
        # load block
        self.block = RBOObject('cabinet')
        self.block.load()
        self.block_quaternion = [0, 0, 0, 1]
        self.block.set_position_orientation(block_position, self.block_quaternion)
    ...
    def feature_count(self, xi):
        ...
        goal_score = -np.linalg.norm(xi[-1, 0:2] -self.goal[0:2])
        #box_move_y = (abs(box_position[1] - self.block_position[1]))
        f = np.array([goal_score, 2.5*(box_move_x)])
        return f
