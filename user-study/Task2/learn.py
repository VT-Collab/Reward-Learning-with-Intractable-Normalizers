...
def main():
	...
	for run in range(n_runs):
		...
		theta_naive_i = mcmc_naive_i(D, n_outer_samples, n_burn, len(theta))
		theta_naive_i = np.mean(theta_naive_i, axis=0)

		# mean
		print("mean - independent")
		theta_mean_i = mcmc_mean_i(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
		theta_mean_i = np.mean(theta_mean_i, axis=0)

		# max
		print("max - independent")
		theta_max_i = mcmc_max_i(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
		theta_max_i = np.mean(theta_max_i, axis=0)

		# double
		print("double - independent")
		theta_double_i = mcmc_double_i(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
		theta_double_i = np.mean(theta_double_i, axis=0)


		# naive
		print("naive - dependent")
		theta_naive_d = mcmc_naive_d(D, n_outer_samples, n_burn, len(theta))
		theta_naive_d = np.mean(theta_naive_d, axis=0)

		# mean
		print("mean - dependent")
		theta_mean_d = mcmc_mean_d(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
		theta_mean_d = np.mean(theta_mean_d, axis=0)

		# max
		print("max - dependent")
		theta_max_d = mcmc_max_d(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
		theta_max_d = np.mean(theta_max_d, axis=0)

		# double
		print("double - dependent")
		theta_double_d = mcmc_double_d(D, n_outer_samples, n_inner_samples, n_burn, xi0, len(theta))
