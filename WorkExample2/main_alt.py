...
def main():
	...
	for run in range(n_runs):
		#generaate a random state from 0 to 1 for the x and y axis position
		...
		a_h, _ = human_action(state,theta, 1000) #theoretical ideal human action given a belief of theta
		a_state = [a_h[0]+state[0], a_h[1]+state[1]]
		# naive
		print("Naive")
		theta_naive = mcmc_naive(state,a_h, n_outer_samples, n_burn)
		theta_naive = np.mean(theta_naive, axis=0) #average value from MH sampling
	
		# mean
		print("mean")
		theta_mean = mcmc_mean(state,a_h, n_outer_samples, n_inner_samples, n_burn)
		theta_mean = np.mean(theta_mean, axis=0)
	
		# max
		print("Max")
		theta_max  = mcmc_max(state,a_h, n_outer_samples, n_inner_samples, n_burn)
		theta_max = np.mean(theta_max, axis=0)
	
		# double
		print("dmh")
		theta_double = mcmc_double(state,a_h, n_outer_samples, n_inner_samples, n_burn)
